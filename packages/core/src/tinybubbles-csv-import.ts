// Generic "Tiny Bubbles CSV" importer for users migrating from apps with no dedicated importer.
...
// Value intentionally NOT rebranded: this string is hashed (via generateDeterministicUUID)
// into every area/project/section/task ID this importer produces. Changing it would make a
// routine re-import of a previously-imported CSV mint new IDs for every row instead of
// matching the existing ones, turning an update into a full duplicate set.
const TINYBUBBLES_CSV_IMPORT_ID_NAMESPACE = 'tinybubbles:csv-import:v1';
const TINYBUBBLES_CSV_AREA_FALLBACK = 'Tiny Bubbles CSV Area';
const TINYBUBBLES_CSV_PROJECT_FALLBACK = 'Tiny Bubbles CSV Import';
const TINYBUBBLES_CSV_IMPORT_SUFFIX = ' (Tiny Bubbles CSV)';

const createTinyBubblesCsvImportId = (kind: 'area' | 'project' | 'section' | 'task', sourceKey: string): string => (
    generateDeterministicUUID(`${TINYBUBBLES_CSV_IMPORT_ID_NAMESPACE}:${kind}:${sourceKey}`)
);

type TinyBubblesCsvFileInput = {
    bytes?: ArrayBuffer | Uint8Array | null;
    fileName: string;
    text?: string | null;
};

type TinyBubblesCsvWarningCounters = {
    duplicateIds: number;
...
export type ParsedTinyBubblesCsvArea = {
    name: string;
    order: number;
    sourceKey: string;
};

export type ParsedTinyBubblesCsvProject = {
...
export type ParsedTinyBubblesCsvSection = {
    name: string;
    order: number;
...
export type ParsedTinyBubblesCsvTask = {
    areaSourceKey?: string;
    assignedTo?: string;
...
export type ParsedTinyBubblesCsvImportData = {
    areas: ParsedTinyBubblesCsvArea[];
    projects: ParsedTinyBubblesCsvProject[];
    sections: ParsedTinyBubblesCsvSection[];
    tasks: ParsedTinyBubblesCsvTask[];
    warnings: string[];
};

export type TinyBubblesCsvImportProjectPreview = {
    areaName?: string;
    name: string;
    taskCount: number;
};

export type TinyBubblesCsvImportPreview = {
    areaCount: number;
    checklistItemCount: number;
    fileName: string;
    projectCount: number;
    projects: TinyBubblesCsvImportProjectPreview[];
    sectionCount: number;
...
export type TinyBubblesCsvImportParseResult = ImportParseResult<ParsedTinyBubblesCsvImportData, TinyBubblesCsvImportPreview>;

export type TinyBubblesCsvImportExecutionResult = ImportExecutionResult & { importedSectionCount: number };
...
const createWarningCounters = (): TinyBubblesCsvWarningCounters => ({
    duplicateIds: 0,
    emptyTitleRows: 0,
    invalidCsvFiles: 0,
    nestedZipFiles: 0,
...
const buildWarnings = (counters: TinyBubblesCsvWarningCounters): string[] => {
...
const detectTinyBubblesCsvDelimiter = (text: string): string => {
...
const countUnknownColumns = (headerRow: string[], counters: TinyBubblesCsvWarningCounters): void => {
...
const resolveStatus = (
    ...
    counters: TinyBubblesCsvWarningCounters
): TaskStatus => {
...
const parseDateCell = (raw: string, counters: TinyBubblesCsvWarningCounters): string | undefined => {
    const value = parseCsvDateValue(raw);
    if (!value && raw.trim()) counters.unparsedDates += 1;
    return value;
};

const parseTimestampCell = (raw: string, counters: TinyBubblesCsvWarningCounters): string | undefined => {
    const value = normalizeEntityTimestamp(raw);
    if (!value && raw.trim()) counters.unparsedDates += 1;
    return value;
};

type TinyBubblesCsvRowSource = {
    documentFingerprint: string;
    kind: 'standalone-file' | 'zip-entry';
    name: string;
};

const parseTinyBubblesCsvRows = (
    csvText: string,
    counters: TinyBubblesCsvWarningCounters,
    source: TinyBubblesCsvRowSource,
    archiveBudget: ImportArchiveBudget,
): ParsedTinyBubblesCsvImportData => {
    const delimiter = detectTinyBubblesCsvDelimiter(csvText);
    ...
    if (!headerIndex.has('TITLE')) {
        throw new Error('Tiny Bubbles CSV is missing the required column: Title');
    }
    countUnknownColumns(rows[0] || [], counters);

    const areasByKey = new Map<string, ParsedTinyBubblesCsvArea>();
    const projectsByKey = new Map<string, ParsedTinyBubblesCsvProject>();
    const sectionsByKey = new Map<string, ParsedTinyBubblesCsvSection>();
    const sectionCountByProject = new Map<string, number>();
    const tasks: ParsedTinyBubblesCsvTask[] = [];
...
const mergeParsedData = (target: ParsedTinyBubblesCsvImportData, source: ParsedTinyBubblesCsvImportData): void => {
...
const bucketKeyForTask = (task: ParsedTinyBubblesCsvTask): string => (
    task.projectSourceKey ? `project:${task.projectSourceKey}`
        : task.areaSourceKey ? `area:${task.areaSourceKey}`
            : 'inbox'
);

const buildPreview = (fileName: string, parsedData: ParsedTinyBubblesCsvImportData): TinyBubblesCsvImportPreview => {
...
export const parseTinyBubblesCsvImportSource = (input: TinyBubblesCsvFileInput): TinyBubblesCsvImportParseResult => {
    const fileName = basename(input.fileName);
    const counters = createWarningCounters();
    const parsedData: ParsedTinyBubblesCsvImportData = { areas: [], projects: [], sections: [], tasks: [], warnings: [] };
    const archiveBudget = createImportArchiveBudget();

    const parseOneCsv = (csvText: string, source: TinyBubblesCsvRowSource): void => {
        const parsed = parseTinyBubblesCsvRows(csvText, counters, source, archiveBudget);
    ...
    try {
        ...
        return {
            ...
            errors: [error instanceof Error && error.message ? error.message : 'Failed to parse the Tiny Bubbles CSV file.'],
        };
    }
...
export const applyTinyBubblesCsvImport = (
    currentData: AppData,
    parsedData: ParsedTinyBubblesCsvImportData,
    options: { now?: Date | string } = {}
): TinyBubblesCsvImportExecutionResult => {
    ...
    const taskSourceKeyFor = (task: ParsedTinyBubblesCsvTask): string => {
    ...
    parsedData.areas.forEach((area) => {
        resolvedIds.area.set(area.sourceKey, createTinyBubblesCsvImportId('area', area.sourceKey));
    });
    ...
    parsedData.projects.forEach((project) => {
        const scopedId = createTinyBubblesCsvImportId('project', project.sourceKey);
        ...
        const colonId = createTinyBubblesCsvImportId('project', colonSourceKey);
        ...
        const legacyId = legacySourceKey
            ? createTinyBubblesCsvImportId('project', legacySourceKey)
            : undefined;
    ...
    parsedData.sections.forEach((section) => {
        const scopedId = createTinyBubblesCsvImportId('section', section.sourceKey);
        ...
        const colonId = createTinyBubblesCsvImportId('section', colonSourceKey);
        ...
        const legacyId = legacySourceKey
            ? createTinyBubblesCsvImportId('section', legacySourceKey)
            : undefined;
    ...
    const historicalTaskCandidatesFor = (sourceKeys: Array<string | undefined>): Array<{ id: string; task: Task }> => {
        ...
        sourceKeys.forEach((sourceKey) => {
            if (!sourceKey) return;
            const id = createTinyBubblesCsvImportId('task', sourceKey);
    ...
    tasksForImport.forEach((task) => {
        const scopedId = createTinyBubblesCsvImportId('task', task.sourceKey);
        ...
        const compatibilityId = task.compatibilitySourceKeys
            ?.map((sourceKey) => createTinyBubblesCsvImportId('task', sourceKey))
            .find((id) => currentTaskById.has(id));
        if (compatibilityId) {
            resolvedIds.task.set(task.sourceKey, compatibilityId);
            return;
        }
        const precedingGlobalId = createTinyBubblesCsvImportId(
            'task',
    ...
    const compatibleIdFor = (
        ...
        sourceKey: string,
    ): string => resolvedIds[kind].get(sourceKey) ?? createTinyBubblesCsvImportId(kind, sourceKey);
    ...
    const applied = applyImport(
        ...
        {
            fallbacks: { area: TINYBUBBLES_CSV_AREA_FALLBACK, project: TINYBUBBLES_CSV_PROJECT_FALLBACK },
            idFor: compatibleIdFor,
            now: options.now,
            suffix: TINYBUBBLES_CSV_IMPORT_SUFFIX,
        }
    );
