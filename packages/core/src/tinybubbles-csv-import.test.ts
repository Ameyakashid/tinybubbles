...
describe('tinybubbles csv import', () => {
    it('parses a full-featured row into the right task, project, section, and area fields', () => {
        ...
        const result = parseTinyBubblesCsvImportSource({ fileName: 'export.csv', text: csv });
    ...
    it('rejects a checklist that exceeds the safe item limit before allocating items', () => {
        ...
        const result = parseTinyBubblesCsvImportSource({
            fileName: 'export.csv',
    ...
    it('errors when the Title column is missing', () => {
        ...
        expect(result.errors).toEqual(['Tiny Bubbles CSV is missing the required column: Title']);
    });
    ...
    it('parses a zipped export and skips unsupported archive entries', () => {
        ...
        const result = parseTinyBubblesCsvImportSource({ fileName: 'export.zip', bytes: archive });
    ...
    it('imports parsed data into areas, projects, sections, and tasks with fresh revisions', () => {
        ...
        const parsedData = parseTinyBubblesCsvImportSource({ fileName: 'export.csv', text: csv }).parsedData as ParsedTinyBubblesCsvImportData;

        const result = applyTinyBubblesCsvImport(mockAppData([], [], []), parsedData, { now: '2026-08-08T12:00:00.000Z' });
    ...
    it('keeps equal project and section names distinct across areas when parsing and applying', () => {
        ...
        const parsed = parseTinyBubblesCsvImportSource({ fileName: 'export.csv', text: csv });
        ...
        const applied = applyTinyBubblesCsvImport(
            mockAppData([], [], []),
            parsed.parsedData as ParsedTinyBubblesCsvImportData,
    ...
    it('reuses IDs from the colon-scoped importer after tuple escaping is introduced', () => {
        ...
        const escaped = parseTinyBubblesCsvImportSource({ fileName: 'export.csv', text: csv })
            .parsedData as ParsedTinyBubblesCsvImportData;
        const colonProjectSourceKey = 'work:north:ops:core';
        const colonSectionSourceKey = 'work:north:ops:core:queue:now';
        const colonScoped: ParsedTinyBubblesCsvImportData = {
            ...escaped,
        ...
        const first = applyTinyBubblesCsvImport(mockAppData([], [], []), colonScoped, {
            now: '2026-08-08T12:00:00.000Z',
        });

        const second = applyTinyBubblesCsvImport(first.data, escaped, {
    ...
    it('reuses a task ID from the escaped project-scoped importer', () => {
        ...
        const parsed = parseTinyBubblesCsvImportSource({ fileName: 'export.csv', text: csv })
            .parsedData as ParsedTinyBubblesCsvImportData;
        const first = applyTinyBubblesCsvImport(mockAppData([], [], []), parsed, {
            now: '2026-08-08T12:00:00.000Z',
        });
        const priorTaskId = generateDeterministicUUID(
            'tinybubbles:csv-import:v1:task:work%3Anorth:ops%3Acore:task%3A1',
        );
        const priorData = {
            ...first.data,
        ...
        const second = applyTinyBubblesCsvImport(priorData, parsed, {
            now: '2026-08-09T12:00:00.000Z',
        });
    ...
    it('migrates a moved task from the project-scoped importer without changing its ID', () => {
        ...
        const beforeParsed = parseTinyBubblesCsvImportSource({ fileName: 'export.csv', text: beforeCsv })
            .parsedData as ParsedTinyBubblesCsvImportData;
        const afterParsed = parseTinyBubblesCsvImportSource({ fileName: 'export.csv', text: afterCsv })
            .parsedData as ParsedTinyBubblesCsvImportData;
        const first = applyTinyBubblesCsvImport(mockAppData([], [], []), beforeParsed, {
            now: '2026-08-08T12:00:00.000Z',
        });
        const targetSeed = parseTinyBubblesCsvImportSource({
            fileName: 'target.csv',
            text: buildCsv(
                ['Title', 'Project', 'ID'],
        ...
        const seeded = applyTinyBubblesCsvImport(first.data, targetSeed, {
            now: '2026-08-08T13:00:00.000Z',
        });
        const priorTaskId = generateDeterministicUUID(
            'tinybubbles:csv-import:v1:task:before:stable-task',
        );
        const priorData = {
            ...seeded.data,
        ...
        const second = applyTinyBubblesCsvImport(priorData, afterParsed, {
            now: '2026-08-09T12:00:00.000Z',
        });
    ...
    it('does not collapse rows at the same position across two CSVs in one ZIP (C1)', () => {
        ...
        const applied = applyTinyBubblesCsvImport(mockAppData([], [], []), result.parsedData as ParsedTinyBubblesCsvImportData, { now: '2026-08-08T12:00:00.000Z' });
        expect(applied.importedTaskCount).toBe(2);
    });

    it('keeps row fallbacks distinct across separately imported standalone CSV files', () => {
        const firstParsed = parseTinyBubblesCsvImportSource({
            fileName: 'first.csv',
            text: buildCsv(['Title', 'Project'], [['First file task', 'Ops']]),
        }).parsedData as ParsedTinyBubblesCsvImportData;
        const secondParsed = parseTinyBubblesCsvImportSource({
            fileName: 'second.csv',
            text: buildCsv(['Title', 'Project'], [['Second file task', 'Personal']]),
        }).parsedData as ParsedTinyBubblesCsvImportData;

        const first = applyTinyBubblesCsvImport(mockAppData([], [], []), firstParsed, {
            now: '2026-08-08T12:00:00.000Z',
        });
        const second = applyTinyBubblesCsvImport(first.data, secondParsed, {
            now: '2026-08-09T12:00:00.000Z',
        });
    ...
    it('keeps no-ID row identity stable when identical CSV content is renamed', () => {
        ...
        const renamedParsed = parseTinyBubblesCsvImportSource({
            fileName: 'renamed.csv',
            text: csv,
        }).parsedData as ParsedTinyBubblesCsvImportData;

        const first = applyTinyBubblesCsvImport(mockAppData([], [], []), firstParsed);
        const second = applyTinyBubblesCsvImport(first.data, renamedParsed);
    ...
    it('reuses IDs created by the preceding filename-based no-ID namespace', () => {
        const parsed = parseTinyBubblesCsvImportSource({
            fileName: 'export.csv',
            text: buildCsv(['Title'], [['Existing filename task']]),
        }).parsedData as ParsedTinyBubblesCsvImportData;
        const legacyFilenameParsed: ParsedTinyBubblesCsvImportData = {
            ...parsed,
        ...
        const first = applyTinyBubblesCsvImport(mockAppData([], [], []), legacyFilenameParsed);
        const second = applyTinyBubblesCsvImport(first.data, parsed);
    ...
    it('reports skipped rows when re-importing after the tasks were deleted (#1011)', () => {
        ...
        const first = applyTinyBubblesCsvImport(mockAppData([], [], []), parsedData, { now: '2026-08-08T12:00:00.000Z' });
        const withDeletion = {
            ...first.data,
        ...
        const second = applyTinyBubblesCsvImport(withDeletion, parsedData, { now: '2026-08-09T12:00:00.000Z' });
    ...
    it('does not duplicate a Section when the same import is applied again (T1)', () => {
        ...
        const first = applyTinyBubblesCsvImport(mockAppData([], [], []), parsedData, { now: '2026-08-08T12:00:00.000Z' });
        const second = applyTinyBubblesCsvImport(first.data, parsedData, { now: '2026-08-09T12:00:00.000Z' });
    ...
    it('recognizes records created by the pre-area-scoped deterministic IDs', () => {
        ...
        const scoped = parseTinyBubblesCsvImportSource({ fileName: 'export.csv', text: csv })
            .parsedData as ParsedTinyBubblesCsvImportData;
        const legacy: ParsedTinyBubblesCsvImportData = {
            ...scoped,
        ...
        const first = applyTinyBubblesCsvImport(mockAppData([], [], []), legacy, {
            now: '2026-08-08T12:00:00.000Z',
        });

        const second = applyTinyBubblesCsvImport(first.data, scoped, {
            now: '2026-08-09T12:00:00.000Z',
        });
    ...
    it('keeps an edited shared legacy task in its current container without duplicating it', () => {
        ...
        const second = applyTinyBubblesCsvImport(editedFirstData, scoped, {
            now: '2026-08-09T12:00:00.000Z',
        });
        ...
        const third = applyTinyBubblesCsvImport(second.data, scoped, {
    ...
    it('clears project ordering when an untouched legacy task is safely moved to Inbox', () => {
        const beforeParsed = parseTinyBubblesCsvImportSource({
            fileName: 'export.csv',
            text: buildCsv(
        ...
        const afterParsed = parseTinyBubblesCsvImportSource({
            fileName: 'export.csv',
    ...
    it('treats a stable legacy task ID as the same task when its scoped container changes', () => {
        ...
        const legacyScoped = parseTinyBubblesCsvImportSource({ fileName: 'legacy.csv', text: legacyCsv })
            .parsedData as ParsedTinyBubblesCsvImportData;
        const legacy: ParsedTinyBubblesCsvImportData = {
            ...legacyScoped,
        ...
        const workScoped = parseTinyBubblesCsvImportSource({ fileName: 'work.csv', text: workCsv })
            .parsedData as ParsedTinyBubblesCsvImportData;

        const second = applyTinyBubblesCsvImport(first.data, workScoped, {
            now: '2026-08-09T12:00:00.000Z',
        });
    ...
    it('parses a tab-delimited file (T2)', () => {
        ...
        const result = parseTinyBubblesCsvImportSource({ fileName: 'export.tsv', text: csv });
    ...
    it('warns when an ID value is duplicated within one import', () => {
        ...
        const applied = applyTinyBubblesCsvImport(
            mockAppData([], [], []),
            result.parsedData as ParsedTinyBubblesCsvImportData,
        );
    ...
    it('reuses the preceding global task ID for an unambiguous explicit CSV ID', () => {
        ...
        const precedingGlobalId = generateDeterministicUUID(
            'tinybubbles:csv-import:v1:task:stable-task',
        );
        const precedingData = {
            ...first.data,
        ...
        const second = applyTinyBubblesCsvImport(precedingData, parsed, {
            now: '2026-08-09T12:00:00.000Z',
        });
    ...
    it('keeps a stable task identity when a corrected re-export moves it between projects', () => {
        ...
        const firstParsed = parseTinyBubblesCsvImportSource({ fileName: 'export.csv', text: firstCsv })
            .parsedData as ParsedTinyBubblesCsvImportData;
        const movedParsed = parseTinyBubblesCsvImportSource({ fileName: 'export.csv', text: movedCsv })
            .parsedData as ParsedTinyBubblesCsvImportData;
        ...
        const second = applyTinyBubblesCsvImport(first.data, movedParsed, {
            now: '2026-08-09T12:00:00.000Z',
        });
