...
private let tinybubblesFocusLockWidgetKind = "TinyBubblesFocusLockWidget"

// Lock screen (accessory family) widget showing the current focused task (#821).
// Reuses TinyBubblesTasksWidgetProvider: accessory families fall through to the
...
struct TinyBubblesFocusLockWidget: Widget {
    let kind: String = tinybubblesFocusLockWidgetKind
    ...
    var body: some WidgetConfiguration {
        StaticConfiguration(kind: kind, provider: TinyBubblesTasksWidgetProvider()) { entry in
            TinyBubblesFocusLockWidgetEntryView(entry: entry)
        }
...
private struct TinyBubblesFocusLockWidgetEntryView: View {
    let entry: TinyBubblesTasksWidgetEntry

    var body: some View {
        if #available(iOSApplicationExtension 16.0, *) {
            TinyBubblesFocusLockView(entry: entry)
        } else {
            EmptyView()
        }
    }
...
private struct TinyBubblesFocusLockView: View {
    let entry: TinyBubblesTasksWidgetEntry
    ...
    var body: some View {
        content
            ...
            .tinybubblesLockWidgetBackground()
    }
...
private extension View {
    @ViewBuilder
    func tinybubblesLockWidgetBackground() -> some View {
