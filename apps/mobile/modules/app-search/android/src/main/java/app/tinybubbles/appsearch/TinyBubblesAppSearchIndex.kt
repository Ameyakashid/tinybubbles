...
private const val DATABASE_NAME = "tinybubbles_search"
private const val NAMESPACE = "tinybubbles"
private const val SCHEMA_TYPE = "TinyBubblesItem"
 ...
 */
data class TinyBubblesAppSearchDoc(
  val id: String,
  val kind: String,
  val title: String,
  val status: String?,
  ...
  companion object {
    fun fromMap(raw: Map<String, Any?>): TinyBubblesAppSearchDoc? {
      ...
      return TinyBubblesAppSearchDoc(
        id = id,
        kind = kind,
        title = title,
 ...
 */
object TinyBubblesAppSearchIndex {
  ...
  private fun toGenericDocument(doc: TinyBubblesAppSearchDoc): GenericDocument {
  ...
  fun upsert(context: Context, docs: List<TinyBubblesAppSearchDoc>, onDone: (Result<Unit>) -> Unit) {
    if (docs.isEmpty()) {
