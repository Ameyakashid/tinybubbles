...
import {
  ...
  matchesTask as taskMatchesSearchTerm,
  parseSearchQuery,
...
/**
 * Filter selections for the mobile picker surfaces: Focus and the task list
 * both hold the same nine selections plus a search box, so they hold them
 * here. Criteria live in core's filter-criteria module; this hook owns only
 * the picker state around it — the tri-state token cycle, the visibility
 * pruning, the active-filter chips, and the saved-filter binding.
 */
