...
data-change, initial sync), and returns `{ hydrated, loadError, lastSyncError,
syncPending, persistError, persistRetrying, requestSync, retryPersistence,
retryLoad }`. `loadError` is set when loading stored data
