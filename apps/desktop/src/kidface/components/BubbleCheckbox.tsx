    ...
    function BubbleCheckbox({ checked, onChange, label, celebrating = false, className }, ref) {
        return (
            <button
                ...
                onClick={onChange}
                className={cn(
                    'relative flex size-[88px] shrink-0 items-center justify-center rounded-full border-[3px] transition-colors duration-200 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-ring/50 active:scale-90',
                    checked
            ...
            <span
                className={cn(
                    'pointer-events-none absolute inset-[-10px] rounded-full bg-success/25 opacity-0 blur-md transition-opacity duration-300',
            ...
            {celebrating && (
                <>
                    ...
                    />
                    <span
                        className="pointer-events-none absolute inset-[-12px] rounded-full border-2 border-success/40 kidface-bubble-ripple-delayed"
                        aria-hidden="true"
                    />
                </>
            )}
                ...
                <span className="pointer-events-none absolute inset-0" aria-hidden="true">
                    {Array.from({ length: 6 }).map((_, index) => (
                        <span
                            key={index}
                            className={cn(
                                'kidface-sparkle size-2 rounded-full bg-focus-star shadow-[0_0_5px_rgba(250,204,21,0.8)]',
