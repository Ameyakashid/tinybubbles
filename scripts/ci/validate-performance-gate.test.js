...
test("CI executes the production-path large-store performance budgets", () => {
  ...
  expect(rootPackage.scripts["test:perf"]).toContain("--filter @tinybubbles/core test:perf");
  ...
  expect(workflow).not.toContain("bun run --filter @tinybubbles/core test:perf");
