...
const makeTempDir = () => {
  const dir = mkdtempSync(join(tmpdir(), 'tinybubbles-api-'));
  tempDirs.push(dir);
  return dir;
};
...
const spawnApi = (
  ...
  return Bun.spawn({
    ...
    env: {
      ...process.env,
      TINYBUBBLES_API_CORS_ORIGIN: options.corsOrigin ?? '',
      TINYBUBBLES_API_TOKEN: options.token ?? '',
    },
  });
};
...
describe('tinybubbles-api', () => {
  test('refuses to start without a bearer token unless auth is dangerously disabled', async () => {
    ...
    expect(stderr).toContain('TINYBUBBLES_API_TOKEN');
  ...
  test('refuses wildcard CORS configuration', async () => {
    ...
    expect(exitCode).toBe(1);
    expect(stderr).toContain('TINYBUBBLES_API_CORS_ORIGIN');
