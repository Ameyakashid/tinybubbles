...
- The MCP task tools accept the full task field set: `mindwtr_add_task` and `mindwtr_update_task` gained checklist, attachments-free file fields, location, area, review date, focus flag, time tracking, and reminder fields that only the apps and cloud API could write before — and new synced fields will appear there automatically instead of needing hand additions. The local and cloud MCP backends now return the same error code for the same mistake (a missing task is `not_found`, not `internal_error`), and self-hosted `/v1/search` reports the true match total and pages correctly past 200 results.
