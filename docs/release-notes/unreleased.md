...
- Desktop: opening a done or archived task from search now actually shows it — the Done and Archive lists expand the group it sits in, scroll to the row and flash it, and say so when an archive filter hiding it had to be cleared. Finished tasks that belong to a project open in Done/Archive instead of the project page, which cannot show them. (#991)
- Calendar: a recurring task with "Show future occurrences in Calendar" on (renamed from "Show next occurrence in Calendar") now paints every occurrence across the visible month, week, or schedule range instead of just the next one — a daily task fills every visible day. Previews stay read-only and device-calendar push is unchanged. Desktop and mobile.
...
- Focus and Next actions: a task whose start date includes a specific time now stays hidden until that time arrives, not just until the day — a task startable at 5:00 PM no longer sits in the morning's list (with Show Top 3 it blocked a slot all day). It appears on its own the moment the time passes. Date-only start dates still show all day, and Daily Review still offers tonight's tasks while you plan the morning. Desktop and mobile. (#995)
