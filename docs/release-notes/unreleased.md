...
- Settings search now finds every setting — the index grew from 44 to 140 rows — and desktop results show as a list of matches with their location ("Clean up quick add text — GTD → Default capture method"). Picking one jumps to the page, opens the containing section, and highlights the row. Mobile search shows the same locations in its results. (#884)
